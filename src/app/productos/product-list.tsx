...
export default function ProductList() {
  ...
  const hasProductsForCategory = useCallback(
    (category: ProductCategory | 'all'): boolean => {
      if (category === 'all') {
  ...
  useEffect(() => {
    if (!hasProductsForCategory(selectedCategory) && !isLoading) {
      console.log(
        `No products found for category: ${selectedCategory}. Fetching...`
      )
      actions.fetchAllProducts()
    }
  ...
  const showCategoryLoading =
    isLoading && !hasProductsForCategory(selectedCategory)
